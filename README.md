...
- Swagger/OpenAPI 导入：上传 OpenAPI 2.0/3.0 JSON 后先预览「新增 / 重复 / 无法解析」三类条目；重复项可逐条选择跳过或替换（替换保留原请求日志关联），存在无法解析条目时禁止提交；确认后仅写入选中条目，部分失败会保留预览以便重试
...
| POST | /api/v1/projects/:projectId/swagger/preview | 预览 OpenAPI 导入（新增/重复/无法解析，不写入） |
| POST | /api/v1/projects/:projectId/swagger/commit | 确认导入（仅写入选中条目，返回逐条失败明细） |
| POST | /api/v1/projects/:projectId/swagger/import | 一键导入（旧接口，新增并跳过重复） |
